...
# The subscriber is only registered if Sentry is properly initialized.

module CspViolationSentrySubscriber
  EVENT_NAME = "csp.violation"

  # Only act on our own event. Rails.event also carries every framework
  # structured event (controller.request_started, active_record.sql, ...),
  # and reporting those here turned each health check into a "CSP Violation".
  def self.csp_violation?(event)
    event[:name] == EVENT_NAME
  end

  def self.emit(event)
    return unless csp_violation?(event)
...
Rails.application.config.after_initialize do
  if defined?(Sentry) && Sentry.initialized?
    Rails.event.subscribe(CspViolationSentrySubscriber) { |event| CspViolationSentrySubscriber.csp_violation?(event) }
